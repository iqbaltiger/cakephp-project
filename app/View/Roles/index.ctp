...
<div class="header bg-light b-b b-light text-right">
	<div class="btn-group">
		...
		<?php echo $this->Html->link(__('Role List'), array('action' => 'index'),array('class' => 'btn btn-default')); ?>
	</div>
	...
	<div class="btn-group">
		...
		<?php echo $this->Html->link(__('Add New Role'), array('action' => 'add'),array('class' => 'btn btn-primary')); ?>
	</div>
</div>
<!-- Panel -->
<div class="widget">
		...
		<table class="table table-striped table-bordered  table-hover">
			...
			<tbody>
				<?php $count=1; foreach ($roles as $role): ?>
					<tr>
						<td><?php echo $count;$count++; //echo h($role['Role']['id']); ?>&nbsp;</td>
						<td>
						...
						<td>
							<?php echo $this->Html->link('<i class="icon-large icon-eye-open"></i>', array('action' => 'view', $role['Role']['id']),array('class'=>'btn btn-rounded btn-small btn-icon btn-primary', 'escape'=>false)); ?>
							<?php echo $this->Html->link('<i class="icon-large icon-edit"></i> ', array('action' => 'edit', $role['Role']['id']),array('class'=>'btn btn-rounded btn-small btn-icon btn-info','escape'=>false)); ?>
							<?php echo $this->Form->postLink('<i class="icon-large icon-trash"></i>', array('action' => 'delete', $role['Role']['id']), array('class'=>'btn btn-rounded btn-small btn-icon btn-danger', 'escape'=>false), __('Are you sure?')); ?>
						</td>
					</tr>
					<?php endforeach; ?>
			</tbody>
