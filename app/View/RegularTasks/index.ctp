...
<div class="header bg-light b-b b-light text-right">
	<div class="btn-group">
		...
		<?php echo $this->Html->link(__('Regular Task List'), array('action' => 'index'),array('class' => 'btn btn-default')); ?>
	</div>
	...
	<div class="btn-group">
		...
		<?php echo $this->Html->link(__('Add New Regular Task'), array('action' => 'add'),array('class' => 'btn btn-primary')); ?>
	</div>
</div>
<!-- Panel -->
<div class="widget">
		...
		<table class="table table-striped table-bordered  table-hover">
			<thead>
				<tr>
											...
											<th><?php echo $this->Paginator->sort('task_name', null, array('class'=>'')); ?></th>
			...
			<tbody>
				<?php foreach ($regularTasks as $regularTask): ?>
					<tr>
						<td><?php echo h($regularTask['RegularTask']['id']); ?>&nbsp;</td>
						<td>
						...
						<td><?php echo h($regularTask['RegularTask']['created']); ?>&nbsp;</td>
						<td><?php echo h($regularTask['RegularTask']['modified']); ?>&nbsp;</td>
						<td><?php echo h($regularTask['RegularTask']['status']); ?>&nbsp;</td>
						<td>
							<?php echo $this->Html->link('<i class="icon-large icon-eye-open"></i>', array('action' => 'view', $regularTask['RegularTask']['id']),array('class'=>'btn btn-rounded btn-small btn-icon btn-primary', 'escape'=>false)); ?>
							<?php echo $this->Html->link('<i class="icon-large icon-edit"></i> ', array('action' => 'edit', $regularTask['RegularTask']['id']),array('class'=>'btn btn-rounded btn-small btn-icon btn-info','escape'=>false)); ?>
							<?php echo $this->Form->postLink('<i class="icon-large icon-trash"></i>', array('action' => 'delete', $regularTask['RegularTask']['id']), array('class'=>'btn btn-rounded btn-small btn-icon btn-danger', 'escape'=>false), __('Are you sure?')); ?>
						</td>
					</tr>
					<?php endforeach; ?>
			</tbody>
